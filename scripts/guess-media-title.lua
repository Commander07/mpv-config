...
local function build_title(info)
    ...
    elseif info.type == "episode" and info.season ~= nil and type(info.season) == "number" and info.episode ~= nil and type(info.episode) == "number" then
        local episode_spec = string.format(opts.episode_spec, info.season, info.episode)
        if opts.show_episode_title and info.episode_title ~= nil and type(info.episode_title) == "string" then
            return string.format(opts.title_format, info.title, episode_spec, info.episode_title)
        end
        return string.format(opts.short_title_format, info.title, episode_spec)
    else
        return info.title
    end
end
